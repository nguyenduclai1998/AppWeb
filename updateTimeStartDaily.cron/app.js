	...
	client.llen("data_service_logs", async function(err, reply) {
		...
		if(reply >= 95000) {
			...
			for(const token of tokenDailyStat) {
				const serviceCodeDailyStat = await db.collection("daily_stat").distinct("service_code", {token:token})
				for(const serviceCode of serviceCodeDailyStat) {
					const dataServices = await db.collection("services").findOne({service_code:serviceCode})
					await db.collection("daily_stat").updateMany({service_code:serviceCode}, {$set: {timeStart:dataServices.created_at}})
					console.log("service code:" + serviceCode + ",timeStart:" +dataServices.created_at )
				}
			}
